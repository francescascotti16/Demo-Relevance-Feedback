...
import numpy as np
import pandas as pd
from f_similarity_metrics_old import get_distance_matrix, get_similarity_matrix
...
def rocchio(df, indexes_positive_initial_images, seed=0,  n_display=25, max_iter=20, alpha=1, beta=1, gamma=1,fun_name="euclidean", k_pos=-1, k_neg=-1,  initial_query=None, use_batch=False):
    ...
    display_df, new_query, time_new_query, total_time_single_step, get_distance_matrix_time, time_create_display, get_similarity_matrix_time = rocchio_single_step(data_df,display_df, relevant_ids,non_relevant_ids, alpha, beta, gamma,fun_name=fun_name, initial_query=initial_query)
    old_query = new_query
    ...
    while num_positive_in_the_display < n_display and iter_count < max_iter and action != []:
        ...
        if use_batch:
            ...
            relevant_ids=[im for im, rel in action if rel == 1]
            non_relevant_ids=[im for im, rel in action if rel == 0]
            display_df, new_query, time_new_query, total_time_single_step, get_distance_matrix_time, time_create_display, get_similarity_matrix_time= rocchio_single_step(data_df,display_df, relevant_ids,non_relevant_ids, alpha, beta, gamma,fun_name=fun_name, initial_query=old_query)
    ...
    res={"map_results":map_results, "ndcg_results":ndcg_results, "recall_results":recall_results, "num_positives_at_iter":num_positives_at_iter, "action_dic":action_dic,"time_new_query":time_new_query}
    return res
