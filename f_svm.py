...
def svm_single_step(data_df,display_df, relevant_ids,non_relevant_ids,initial_scores=None,alpha=0, beta=1):
    ...
    new_scores_1,time_distance_from_hyperplane, total_time_score_computation=svm_score(data_df, relevant, non_relevant)
    new_scores = alpha*old_scores+beta*new_scores_1
    display_df, total_time_display = create_display(data_df, new_scores, n_display, is_ascending=False)
    end_time = datetime.now()
...
def svm(df, indexes_positive_initial_images, seed=0,  n_display=25, max_iter=20,  k_pos=-1, k_neg=-1,  use_batch=False,alpha=0, beta=1):
    ...
    display_df, new_scores, total_time_single_step, time_distance_from_hyperplane, total_time_score_computation= svm_single_step(data_df,display_df, relevant_ids,non_relevant_ids+fake_non_relevant_ids,initial_scores=None,alpha=alpha, beta=beta)
    ...
    while num_positive_in_the_display < n_display and iter_count < max_iter and action != []:
        ...
        if use_batch:
            ...
            relevant_ids=[im for im, rel in action if rel == 1]
            non_relevant_ids=[im for im, rel in action if rel == 0]        
            display_df, new_scores, total_time_single_step, time_distance_from_hyperplane, total_time_score_computation= svm_single_step(data_df,display_df, relevant_ids,non_relevant_ids+fake_non_relevant_ids, initial_scores=old_scores,alpha=alpha, beta=beta)
