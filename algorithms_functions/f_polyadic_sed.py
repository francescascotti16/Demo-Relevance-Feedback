...
def get_sed_logscale_sim(data_df,entropy_dict,relevant_ids,non_relevant_ids,old_score,alpha,beta,gamma,dict):
    ...
    for el in relevant_ids:
        ...
        n_pos+=1
    precomputed_entropy_avg_pos=shannon_entropy((precomputed_sum_pos/n_pos) )
    ...
    for el in non_relevant_ids: 
        ...
        n_neg+=1
    precomputed_entropy_avg_neg=shannon_entropy((precomputed_sum_neg/n_neg) ) 
...
def get_sed_logscale_sim_vec(data_df,entropy_dict,query):
    ...
    query_complexity= np.exp(shannon_entropy(query))# complexity 
...
def poly_sed_logscale_single_step(data_df,display_df, relevant_ids,non_relevant_ids,precomputed_dict_initial=None, alpha=0.7, beta=0.7, gamma=0.4, initial_query=None, initial_scores=None,entropy_dict=None):
    ...
    new_scores, precomputed_dict= get_sed_logscale_sim(data_df,entropy_dict,relevant_ids,non_relevant_ids,old_scores,alpha,beta,gamma,precomputed_dict)
...
def polyquery_sed_logscale(df,first_display,  n_display=25, max_iter=20, alpha=1, beta=0.7, gamma=0.7, k_pos=-1, k_neg=-1,  initial_query=None, use_batch=False,no_reclick_image=False):
    initial_query=initial_query.reshape(-1, 1)
    '''
    Parameters
    ----------
    ...
    display_df= first_display
    action, list_display_relevance = get_actions(display_df, groundtruth_dic, already_selected_images, k_pos, k_neg,no_reclick_image=no_reclick_image)
    ...
    display_df, new_score,precomputed_dict,entropy_dict= poly_sed_logscale_single_step(data_df,display_df, relevant_ids,non_relevant_ids,precomputed_dict_initial=None, alpha=alpha, beta=beta, gamma=gamma, initial_query=initial_query,entropy_dict=None)
    old_score = new_score
    ...
    while num_positive_in_the_display < n_display and iter_count < max_iter and action != []:
        ...
        if use_batch:
            relevant_ids+=[im for im, rel in action if rel == 1]
            non_relevant_ids+=[im for im, rel in action if rel == 0]
            display_df, new_score,precomputed_dict,entropy_dict= poly_sed_logscale_single_step(data_df,display_df, relevant_ids,non_relevant_ids,precomputed_dict_initial=None, alpha=alpha, beta=beta, gamma=gamma, initial_query=initial_query,entropy_dict=entropy_dict)
        else:
            relevant_ids=[im for im, rel in action if rel == 1]
            non_relevant_ids=[im for im, rel in action if rel == 0]
            display_df, new_score,precomputed_dict,entropy_dict= poly_sed_logscale_single_step(data_df,display_df, relevant_ids,non_relevant_ids,precomputed_dict_initial=precomputed_dict, alpha=alpha, beta=beta, gamma=gamma, initial_scores=old_score,entropy_dict=entropy_dict)
