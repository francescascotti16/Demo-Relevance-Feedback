...
import numpy as np
import pandas as pd
from functions_similarity_metrics import compute_similarity_score,get_distance_matrix, get_similarity_matrix, softmax, logistic, complexity
...
def polyquery(df, indexes_positive_initial_images, seed=0,  n_display=25, max_iter=20, alpha=1, beta=0.7, gamma=0.7, fun_name="euclidean", k_pos=-1, k_neg=-1,  initial_query=None, use_batch=False):
    ...
    display_df, new_scores, time_score_relevant, time_score_non_relevant, total_time= poly_single_step(data_df,display_df, relevant_ids,non_relevant_ids,alpha=alpha, beta=beta, gamma=gamma,fun_name=fun_name, initial_query=initial_query)
    ...
    while num_positive_in_the_display < n_display and iter_count < max_iter and action != []:
        ...
        if use_batch:
            ...
            relevant_ids=[im for im, rel in action if rel == 1]
            non_relevant_ids=[im for im, rel in action if rel == 0]
            display_df, new_scores, time_score_relevant, time_score_non_relevant, total_time= poly_single_step(data_df,display_df, relevant_ids,non_relevant_ids,alpha=alpha, beta=beta, gamma=gamma,fun_name=fun_name, initial_scores=old_score)
