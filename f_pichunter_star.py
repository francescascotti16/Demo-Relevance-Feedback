...
from f_similarity_metrics_old import  get_similarity_matrix_temperature
...
def pichunter(df,indexes_positive_initial_images,seed=0, n_display=25, max_iter=20, fun_name="softmin", k_pos=-1, k_neg=0, temperature=1, initial_prob=None, use_batch=False): 
    ...
    display_df, new_prob_values, total_time_single_step, time_display, time_get_similarity= pichunter_single_step(data_df,display_df, relevant_ids,non_relevant_ids,fun_name=fun_name, initial_prob=old_prob, temperature=temperature)
    old_prob = new_prob
