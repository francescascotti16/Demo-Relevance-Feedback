...
def polyquery_score(old_scores, data_df, relevant, non_relevant, alpha, beta, gamma, fun_name):
    ...
    def compute_centroid(scores):
        if scores.shape[1] == 0:
            return np.zeros((old_scores.shape[0], 1))
        return np.mean(scores, axis=1).reshape(-1, 1)

    centroid_relevant = compute_centroid(relevant)
    centroid_non_relevant = compute_centroid(non_relevant)
    
    relevant_scores, total_time_get_matrix_relevant,time_sed,total_time_sed_matrix, time_jsd_relevant = get_similarity_matrix(centroid_relevant, data_df, fun_name=fun_name)
    
    non_relevant_scores, total_time_get_matrix_relevant,time_sed_non_relevant,total_time_sed_matrix_non_relevant, time_jsd_non_relevant= get_similarity_matrix(centroid_non_relevant, data_df, fun_name=fun_name)

    new_scores = alpha * old_scores.reshape(-1, 1) + beta * relevant_scores - gamma * non_relevant_scores

    end_time = datetime.now()
    total_time_score = end_time - start_time
    return new_scores, total_time_score, relevant_scores, total_time_get_matrix_relevant,time_sed,total_time_sed_matrix, time_jsd_relevant, non_relevant_scores, total_time_get_matrix_relevant,time_sed_non_relevant,total_time_sed_matrix_non_relevant, time_jsd_non_relevant
def poly_single_step2(data_df,display_df, relevant_ids,non_relevant_ids, alpha=1, beta=0.7, gamma=0.7,fun_name="euclidean", initial_query=None, initial_scores=None):
    '''
    Parameters
    ----------
    data_df : DataFrame
    ...
    if old_scores is None:
        if initial_query is None:
            ...
            old_scores ,time_get_similarity= get_similarity_matrix(initial_query,data_df, fun_name=fun_name)[0]#FIXME 
    ...
    new_scores, total_time_score, relevant_scores, total_time_get_matrix_relevant,time_sed,total_time_sed_matrix, time_jsd_relevant, non_relevant_scores, total_time_get_matrix_relevant,time_sed_non_relevant,total_time_sed_matrix_non_relevant, time_jsd_non_relevant = polyquery_score(old_scores, data_df, relevant, non_relevant, alpha, beta, gamma,fun_name=fun_name)
    ...
    return display_df, new_scores,total_time_single_step,  total_time_score, relevant_scores, total_time_get_matrix_relevant,time_sed,total_time_sed_matrix, time_jsd_relevant, non_relevant_scores, total_time_get_matrix_relevant,time_sed_non_relevant,total_time_sed_matrix_non_relevant, time_jsd_non_relevant
...
def polyquery(df, indexes_positive_initial_images, seed=0,  n_display=25, max_iter=20, alpha=1, beta=0.7, gamma=0.7, fun_name="euclidean", k_pos=-1, k_neg=-1,  initial_query=None, use_batch=False):
    ...
    display_df, new_score= poly_single_step2(data_df,display_df, relevant_ids,non_relevant_ids,alpha=alpha, beta=beta, gamma=gamma,fun_name=fun_name, initial_query=initial_query)
    old_score = new_score
    ...
    while num_positive_in_the_display < n_display and iter_count < max_iter and action != []:
        ...
        if use_batch:
            ...
            relevant_ids=[im for im, rel in action if rel == 1]
            non_relevant_ids=[im for im, rel in action if rel == 0]
            display_df, new_score= poly_single_step2(data_df,display_df, relevant_ids,non_relevant_ids,alpha=alpha, beta=beta, gamma=gamma,fun_name=fun_name, initial_scores=old_score)
